...
const dailySteps = [
  ...
  },
  {
    ...
    description: 'During founder onboarding we set up a receipts alias with you, so invoice emails get matched to your Stripe data instead of piling up.',
...
export default function ReconAgentPage() {
  ...
  // Demo briefing generated by the actual recon-agent engine (ops/recon-agent)
  // at build time — the page shows real engine output, not invented numbers.
  const [demoReport, setDemoReport] = useState(null);

  useEffect(() => {
    let cancelled = false;
    fetch('/recon-demo-report.json', { cache: 'no-store' })
      .then((r) => (r.ok ? r.json() : null))
      .then((payload) => {
        if (!cancelled && payload?.totals && Array.isArray(payload?.flags)) {
          setDemoReport(payload);
        }
      })
      .catch(() => {});
  ...
  // When the engine-generated demo report is available, every number in the
  // briefing panel comes from it; otherwise fall back to static copy.
  const displayReportLines = demoReport
    ? [
        { label: 'Payments processed', value: String(demoReport.totals.payments), tone: 'text-white' },
        { label: 'Money that looks off', value: demoReport.payouts.find((p) => !p.matched)?.deltaDisplay || '$0.00', tone: 'text-amber-300' },
        { label: 'Refunds to review', value: String(demoReport.totals.refunds), tone: 'text-rose-300' },
        { label: 'Failed payments flagged', value: String(demoReport.totals.paymentsFailed), tone: 'text-cyan-300' },
      ]
    : reportLines;

  const severityTone = {
    urgent: 'border-rose-400/30 bg-rose-500/10 text-rose-100',
    review: 'border-amber-400/30 bg-amber-500/10 text-amber-100',
    info: 'border-cyan-400/30 bg-cyan-500/10 text-cyan-100',
  };
  const timelineCards = demoReport
    ? [
        {
          time: '6:31 AM',
          title: 'The math is done',
          detail: demoReport.plainEnglish[0],
          tone: 'border-emerald-400/30 bg-emerald-500/10 text-emerald-100',
        },
        ...demoReport.flags.slice(0, 2).map((flag, idx) => ({
          time: idx === 0 ? '6:33 AM' : '6:34 AM',
          title: flag.title,
          detail: flag.detail,
          tone: severityTone[flag.severity] || severityTone.info,
        })),
      ]
    : [
        {
          time: '6:31 AM',
          title: 'The Math is Done',
          detail: '148 Stripe payments were reconciled against the day’s payout.',
          tone: 'border-cyan-400/30 bg-cyan-500/10 text-cyan-100',
        },
        {
          time: '6:33 AM',
          title: '3 things need a quick look',
          detail: 'A payout total looks short, and 2 late refunds explain why.',
          tone: 'border-amber-400/30 bg-amber-500/10 text-amber-100',
        },
        {
          time: '6:36 AM',
          title: 'Summary is ready',
          detail: 'Your 60-second morning briefing is ready to read.',
          tone: 'border-emerald-400/30 bg-emerald-500/10 text-emerald-100',
        },
      ];
  ...
  return (
    <main className="eb28-appbuilder font-body relative min-h-screen overflow-hidden text-slate-100">
          ...
          <div className="grid gap-12 lg:grid-cols-[minmax(0,1.1fr)_28rem] lg:items-end">
            <div className="max-w-3xl">
              ...
              <p className="mt-6 max-w-2xl text-lg leading-8 text-slate-300 sm:text-xl">
                Recon Agent checks your Stripe account every morning, does the payout math for you, and hands you a 60-second plain-English briefing on exactly where your money went — and why.
              </p>
              ...
              <div className="relative">
                <div className="flex items-center justify-between">
                  ...
                  <div className="rounded-full border border-emerald-400/20 bg-emerald-500/10 px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] text-emerald-200">
                    {demoReport ? 'Live engine output' : 'Ready'}
                  </div>
                </div>

                <div className="mt-6 grid grid-cols-2 gap-3">
                  {displayReportLines.map((line) => (
                ...
                {demoReport && (
                  <p className="mt-4 text-center text-[11px] leading-5 text-slate-500">
                    Every number above was generated by the Recon Agent engine running in demo mode —
                    the same code that runs against your account.
                  </p>
                )}
              </div>
            </div>
      ...
      <section className="px-4 py-16 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl rounded-[2.5rem] border border-white/10 bg-slate-950/60 p-8 backdrop-blur md:p-12">
          <div className="mb-10 max-w-2xl">
            <p className="text-sm font-semibold uppercase tracking-[0.22em] text-cyan-200">Under the hood</p>
            <h2 className="font-brand mt-4 text-3xl font-bold text-white sm:text-4xl">Built so it can look, but never touch.</h2>
            <p className="mt-4 text-base leading-7 text-slate-300">
              The fastest way to lose your trust would be to ask for more access than the job needs. So we don&rsquo;t.
            </p>
          </div>
          <div className="grid gap-6 md:grid-cols-3">
            <div className="rounded-[1.75rem] border border-white/10 bg-white/[0.03] p-6">
              ...
              <h3 className="font-brand mt-5 text-lg font-bold text-white">Read-only by design</h3>
              <p className="mt-3 text-sm leading-7 text-slate-300">
                Recon Agent runs on a restricted, read-only Stripe key. The engine literally refuses to start
                if it is handed a full-access secret key. It can see activity — it can never move money.
              </p>
            </div>
            <div className="rounded-[1.75rem] border border-white/10 bg-white/[0.03] p-6">
              ...
              <h3 className="font-brand mt-5 text-lg font-bold text-white">Your data stays yours</h3>
              <p className="mt-3 text-sm leading-7 text-slate-300">
                Your reports are built for you and delivered to you. Your numbers are never sold, shared,
                or used to train anything. The CSV is yours to hand your bookkeeper.
              </p>
            </div>
            <div className="rounded-[1.75rem] border border-white/10 bg-white/[0.03] p-6">
              ...
              <h3 className="font-brand mt-5 text-lg font-bold text-white">Kill it any time</h3>
              <p className="mt-3 text-sm leading-7 text-slate-300">
                Revoke the key in your Stripe dashboard and access ends that second. Cancel the plan any
                month — you keep every briefing and export you ever received.
              </p>
            </div>
          </div>
        </div>
