...
function buildConsultantReply(userMessage) {
  ...
  if (SMALL_PROJECT_KEYWORDS.some((keyword) => text.includes(keyword))) {
    return `Thomas Custom Homes focuses on custom homes, major renovations, additions, and larger kitchen or bath projects rather than small handyman work. If the scope is larger and it is in Brevard County, call ${THOMAS_PHONE_DISPLAY} or use the estimate form below.`;
  }
  ...
  if (
    ...
    return `Pricing depends on scope, structural work, finish level, and timeline. The fastest way to get useful pricing direction is to send the project type, budget range, and desired completion date through the request form or call ${THOMAS_PHONE_DISPLAY}.`;
  }
                      ...
                      <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 gap-4">
                          {selectedProject.gallery.map((imageUrl, index) => (
                            <div
                              key={imageUrl}
                              className="aspect-video overflow-hidden rounded-2xl shadow-sm"
                            >
                              <img
                                src={imageUrl}
    ...
    <footer className="border-t border-slate-200 bg-slate-50 pb-10 pt-20">
      <div className="mx-auto max-w-7xl px-6">
          ...
          <div>
            ...
            <ul className="space-y-4 text-slate-500">
              ...
              <li className="flex items-center gap-3">
                ...
                <a href={`tel:${THOMAS_PHONE}`} className="transition-colors hover:text-brand-primary">
                  {THOMAS_PHONE_DISPLAY}
                </a>
              </li>
            </ul>
        ...
        <div className="flex flex-col items-center justify-between gap-4 border-t border-slate-200 pt-10 text-sm text-slate-400 md:flex-row">
          <p>&copy; {new Date().getFullYear()} Thomas Custom Homes Inc. All rights reserved.</p>
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[100] flex flex-col items-end">
      ...
      >
        ...
        {!isOpen ? (
          <span className="absolute -right-1 -top-1 h-4 w-4 animate-ping rounded-full bg-brand-accent" />
        ) : null}
...
export default function ThomasCustomHomesHomepageClassic() {
  return (
